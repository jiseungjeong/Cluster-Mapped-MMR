    ...
    for i, test_item in enumerate(test_data):
        try:
            ...
            if method_name == "zero-shot":
                ...
                prompt = f"""Answer the following multiple choice question. Think step by step and provide your reasoning.

Question: {test_item["question"]}

Choices:
A) {test_item["choices"]["A"]}
B) {test_item["choices"]["B"]}
C) {test_item["choices"]["C"]}
D) {test_item["choices"]["D"]}
E) {test_item["choices"]["E"]}

Think through this step by step and then provide your final answer as a single letter (A, B, C, D, or E).
...
def run_all_methods_comparison(dataset_name="commonsenseqa", num_test_samples=100):
    """
    모든 방법론에 대해 비교 실험 실행
...
def main():
    """메인 함수"""
    logger.info("Starting CommonsenseQA Methods Comparison...")
    ...
    # 결과 분석 및 시각화
    analysis = analyze_and_visualize_results(all_results, "commonsenseqa", "./analysis")

    logger.info("\nCommonsenseQA Methods Comparison completed!")
